  ...
  <div>
    ...
    >
      <!--
       lazy 代表是否懶加載
       :load="load" 懶加載的回調
       :tree-props 固定寫法，是否是樹型，前面有沒有>
       :row-key="getRowKey" 設置表格當中每行標示數據key
       :row-class-name 為我們的行設置單獨的樣式
     -->
      <el-table-column
        prop="id"
        label="分类ID"
  ...
  methods: {
    ...
    */
    async load(row, treeNode, resolve) {
      ...
      if (row.level === 1) {
        const categorys2 = await this.getCategory1s2(row)
        resolve(categorys2)
      } else if (row.level === 2) {
        const categorys3 = await this.getCategory1s3(row)
        resolve(categorys3)
      }
    },
    ...
    */
    getCategory1s2(row) {
      return this.$API.category.getCategory1s2(row.id).then(result => {
    ...
    */
    async getCategory1s3(row) {
      const result = await this.$API.category.getCategory1s3(row.id)
